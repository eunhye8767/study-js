 ...
 *  protected를 달아놓으면
 *    1. private 이거랑 똑같은데
 *    2. extends 된 class 안에서도 사용가능하게 약간 보안을 풀어줍니다.
 *
 *  class User {
      protected x = 10;
    }
 ...
 *
 *  class 하나 안에서만 쓸 수 있는 속성을 만들고 싶으면
 *  private 이걸 쓰도록 합시다.
 *
 *  class 여러개 만들 일이 없으면 쓸모없습니다. *
 *  ★★★★★
 *
 *  - private, protected 붙이면 class {} 안에서만 사용이 가능.
 *  - private 가 붙은 변수는 extends 해도 사용할 수 없었다.
 *    protected 는 현재 class {} 안에서, extends 된 class {} 안에서 사용 가능.
 */
 ...
 *
 *  우리가 class { } 안에 집어넣는 변수, 함수 이런건
 *  전부 class로 부터 새로 생성되는 object (일명 instance) 에 부여됩니다.
 *  근데 class에 직접 변수나 함수를 부여하고 싶으면
 *  static 키워드를 왼쪽에 붙여주면 됩니다.
 *
 *  class User {
      x = 10;
      y = 20;
 ...
 *    3. 근데 여기서 js 라는 단어가 중요할 것 같아서
 *       static skill 이 곳에다가 메모해놓고 그걸 사용했습니다.
 *    4. 이제 자식들은 철수.intro 이렇게 사용할 때 마다
 *       'js 전문가입니다~' 를 출력해줍니다.
 *
 *  근데 갑자기 skill을 좀 변경하고 싶은 겁니다.
 *  철수 이후로 생산되는 자식들은 'js 전문가입니다~'가 아니라
 *  'python 전문가입니다' 를 달고 나오게 하고 싶은 것임
 *
 *  그럴 때 class 내부를 직접 js -> python 이렇게 수정해도 되지만
 *  class가 멀리 떨어져있거나 다른 파일에 있을 경우 귀찮습니다.
 *  다행히 static 키워드로 만들어놨기 때문에 그걸 수정해버려도 됩니다.
 *
 *  User2.skill을 저렇게 수정해버리면
 ...
 *  ★★★★★
 *  수정하고 싶으면 private 쓰고
 *  그 다음에 수정함수를 만들어서 사용하는게 더 안전한 방법입니다.
 *  ★★★★★
 *
 */
var User2 = /** @class */ (function () {
    function User2() {
        // 부모 기준으로 가져와야 사용할 수 있는 static
 ...
 *
 *  저렇게 User.addOne() 쓸 때마다
 *  x가 증가하는 함수는 어떻게 만들 수 있을까요?
 *
 ...
 *
 *  html css 기초학력이 흔들리는 분들은
 *  좌절을 느낄 수 있는데 생각보다 별거 아닙니다.
 *
 *  그리고 심심하면 타입지정도 해봅시다.
 */
/**
 *  1. constructor를 이용해서 새로뽑는 object 들은
 *     width, height, color를 입력할 수 있게 만들었습니다.
 *
 *  2. 자식들은 draw()를 쓰면
 *     (1) 0과 1사이의 무작위 숫자를 뽑습니다. 그걸 변수 a에 저장해둡니다.
 *
 *     (2) <div>를 디자인합니다. 근데 <div>박스의 폭, 높이, 색상은
 *         constructor로 입력한 것들을 활용합니다.
 *
 *     (3) <div> 박스의 위치는 left, right 속성을 이용해서
 *         0~400px 사이로 무작위로 배치합니다.
 *
