 ...
 *    조건 2) 함수 실행이 끝나지 않아야합니다 
 *           (전문용어로 endpoint가 없어야합니다)
 *  
 *  그런 함수에 붙일 수 있는 타입니다. 
 *  실은 조건1, 2는 같은 소리인데 모든 자바스크립트 함수 맨 밑엔 
 *  return undefined 라는 숨겨진 코드를 가지고 있습니다. 
 *  
 *  그래서 조건2가 맞으면 1도 맞음 
 * 
 *  function 함수(){
...
/**
 *  아래 함수엔 붙일 수 있습니다. 
 *  while 문법은 ( ) 소괄호안의 조건식이 true일 경우 
 *  계속 내부 코드를 실행해라~ 라는 뜻입니다.
 *  
 *  무한히 실행되기 때문에 끝이안나죠? 
 *  그래서 never 타입을 사용가능합니다. 
 */
function 함수() :never{
  while ( true ) {
    console.log(123)
...
/**
 *  아래와 같은 함수에도 붙일 수 있습니다. 
 *  throw new Error() 문법은 그냥 강제로 에러내라~ 라는 뜻인데
 *  에러가 나면 전체 코드실행이 중단되니까 
 *  2번 조건도 나름 충족하는 것이기 때문에 never를 사용가능합니다.
 */
function 함수2() :never{
 ...
 * 
 *  아래 함수는 뭔가 이상한 함수입니다.
 *  지금 narrowing을 이용해서 파라미터의 타입이 string 이면 
 *  뭔가 해달라고 써놨는데 else 문이 존재합니다. 
 * 
 *  타입이 string이 아닐 경우 이거 해달라는 뜻입니다.
 *  근데 else문은 말이 안되죠? 
 *  지금 파라미터가 string 밖에 못들어오는데 말입니다.
 * 
 *  이런 잘못된 narrowing을 사용했을 때 파라미터의 타입이 never로 변합니다. 
 *  파라미터에 마우스 올려보셈
 * 
