...
- 투두리스트 구현 => 작업 : practice / 수업자료 : result
...
- 실제 프로덕션에서는 사용하지 않음
- npm을 통해 설치 가능
- npm 설치 후, `json-server --watch db.json` 실행
- 로컬서버 종료 단축키 : `control` + `c`
- https://github.com/typicode/json-server#plural-routes<br />`/todos/1`처럼 뒤에 id(`1`) 값을 입력하면 페이지로 이동이 된다. => http://localhost:3000/todos/1
